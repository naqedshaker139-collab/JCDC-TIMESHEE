...
from src.models.timesheet import Timesheet, TimesheetDay
...
def _calculate_hours(row: TimesheetDay):
    ...
    # Rule: first 10 hours regular, rest OT – adjust if needed
...
def _serialize_timesheet(ts: Timesheet):
    ...
    return {
        ...
        # v1: we are not using approval workflow; rely on status only.
        "approval": None,
    }
...
def approve_timesheet(timesheet_id: int):
    data = request.get_json() or {}
    comment = data.get("comment") or ""  # reserved for future use
